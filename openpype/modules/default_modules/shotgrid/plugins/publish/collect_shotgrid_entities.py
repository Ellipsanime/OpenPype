...
class CollectShotgridEntities(pyblish.api.ContextPlugin):
    ...
    def process(self, context):
        ...
        avalon_asset = os.getenv("AVALON_ASSET")
        avalon_task = os.getenv("AVALON_TASK")

        sg_project = _get_shotgrid_project(avalon_project)
        sg_task = _get_shotgrid_task(avalon_project, avalon_asset, avalon_task)
        sg_entity = _get_shotgrid_entity(avalon_project, avalon_asset)

        if sg_project:
            context.data["shotgridProject"] = sg_project
            self.log.info("Collected correspondig shotgrid project : {}".format(sg_project))

        if sg_task:
            context.data["shotgridTask"] = sg_task
            self.log.info("Collected correspondig shotgrid task : {}".format(sg_task))
...
def _get_shotgrid_project(avalon_project):
    proj_settings = _get_shotgrid_project_settings(avalon_project)
...
def _get_shotgrid_task(avalon_project, avalon_asset, avalon_task):
    sg_col = _get_shotgrid_collection(avalon_project)
    shotgrid_task_hierarchy_row = sg_col.find_one({"type": "Task", "_id": avalon_task, "parent": {"$regex": ".*," + avalon_asset + ","}})
...
def _get_shotgrid_entity(avalon_project, avalon_asset):
    sg_col = _get_shotgrid_collection(avalon_project)
    shotgrid_entity_hierarchy_row = sg_col.find_one({"_id": avalon_asset})
    if shotgrid_entity_hierarchy_row:
        return {"type": shotgrid_entity_hierarchy_row['type'], "id": shotgrid_entity_hierarchy_row['src_id']}
    return {}
