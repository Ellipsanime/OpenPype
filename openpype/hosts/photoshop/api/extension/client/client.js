    ...
    RPC.addRoute('Photoshop.parent_layer', function (data) {
        log.warn('Server called route "parent_layer":', data);
        return runEvalScript("parentLayer('"+data.layer_name+"', " +
                                           +data.parent_id +")")
            .then(function(result){
                log.warn("parent_layer: " + result);
                return result;
        });
    });

    RPC.addRoute('Photoshop.move_at', function (data) {
        log.warn('Server called route "move_at":', data);
        return runEvalScript("moveAt('"+ data.layer_name  + "', "
                                       + data.new_index   + ", "
                                       + data.adjustement + ")")
            .then(function(result){
                log.warn("parent_layer: " + result);
