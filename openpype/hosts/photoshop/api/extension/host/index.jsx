...
function parentLayer(layer_name, parent_id){
    var ref = new ActionReference();

    ref.putIdentifier(charIDToTypeID("Lyr "), parent_id)
    parent_index = executeActionGet(ref).getInteger(charIDToTypeID("ItmI"))
    moveAt(layer_name, parent_index - 1, false)
}

function moveAt(layer_name, new_index, adjustement){
    var move_desc = new ActionDescriptor();

    var ref_artLayer = new ActionReference();
    ref_artLayer.putName( charIDToTypeID( "Lyr " ), layer_name );
    move_desc.putReference( charIDToTypeID( "null" ), ref_artLayer );

    var ref_layerSet = new ActionReference();
    ref_layerSet.putIndex( charIDToTypeID( "Lyr " ), new_index );
    move_desc.putReference( charIDToTypeID( "T   " ), ref_layerSet );

    move_desc.putBoolean( charIDToTypeID( "Adjs" ), adjustement );
